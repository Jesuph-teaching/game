...
export default class Game extends GameEventEmitter implements GameState {
	...
	loadEvents() {
		...
		this.on('playerLoggedIn', (socketId, cardsCollection, roomId) => {
			Game.io.to(socketId).emit(SocketEvent.PlayerLoggedIn, cardsCollection, roomId);
		});
		this.on('gameStateUpdated', (state) => {
			console.log('Game Updated');
			if (state.to) Game.io.to(state.to).emit('gameUpdated', state);
			else Game.io.emit(SocketEvent.GameUpdated, state);
		});
		this.on('playerLoggedOut', (socketId) => {
			Game.io.to(socketId).emit(SocketEvent.PlayerLoggedOut);
		});
		this.on('errorHappened', (message) => {
			console.log('Game Error', message);
			Game.io.emit(SocketEvent.Error, message);
		});
		this.on('playerAlreadyLoggedOut', (socketId, state) => {
			Game.io.to(socketId).emit(SocketEvent.PlayerLoggedOut);
			Game.io.to(socketId).emit(SocketEvent.GameUpdated, state);
		});
		this.on('playerWon', (socketId, player) => {
			console.log('player', player.username, 'won');
			Game.io.to(socketId).emit(SocketEvent.PlayerWon, player);
		});
		this.on('playerLost', (socketId, player) => {
			console.log('player', player.username, 'lost');
			Game.io.to(socketId).emit(SocketEvent.PlayerLost, player);
		});
	}
}
